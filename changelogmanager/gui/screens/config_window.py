...
class ConfigWindow(tk.Toplevel):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, controller: AppController) -> None:
        ...
        self.format_var = tk.StringVar(
            value=(
                ...
                if (
                    existing_path
                    and config_format_from_path(existing_path) == "pyproject"
                )
        ...
        self.row(
            ...
            ttk.Combobox(
                ...
                state="readonly",
                values=["pyproject.toml", "changelogmanager.toml"],
                width=28,
            ),
        )
        self.row(
        ...
        self.row(
            body, "Enforce preamble:", ttk.Checkbutton(body, variable=self.enforce_var)
        )
        self.row(
            body,
            "Default component:",
        ...
        self.row(
            body,
            "Default changelog:",
            ttk.Entry(body, textvariable=self.changelog_var, width=30),
        )
    ...
    def row(self, parent, label, widget) -> None:  # type: ignore[no-untyped-def]
        index = getattr(self, "next_row", 0)
        ttk.Label(parent, text=label).grid(
            row=index, column=0, sticky="w", pady=3, padx=(0, 8)
        )
    ...
    def save(self) -> None:
        ...
        answers: dict[str, object] = {
            ...
            "versioning_scheme": self.label_to_scheme[self.scheme_var.get()],
        ...
        updated = build_updated_config(self.base_config, answers)

        if (
            self.existing_path
            and config_format_from_path(self.existing_path) == config_format
        ):
            target = self.existing_path
        else:
        ...
        try:
            ...
            messagebox.showerror(
                "Config save failed", str(getattr(exc, "message", exc))
            )
            return
