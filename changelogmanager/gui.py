...
class ChangelogManagerGUI:  # pylint: disable=too-many-instance-attributes,too-few-public-methods
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.auto_run_done: set[str] = set()
        self.tab_frames: dict[str, ttk.Frame] = {}
        self.output_widgets: dict[str, scrolledtext.ScrolledText] = {}
        self.help_text_widget: tk.Text | None = None
        self.changelog_view: scrolledtext.ScrolledText | None = None
        self.current_command: str = COMMANDS[0]
        self.notebook: ttk.Notebook

        self.build_layout()
        self.show_help(self.current_command)
    ...
    def build_top_panel(self) -> None:
        ...
        ttk.Button(row1, text="Browse…", command=self.browse_input_file).pack(
            side=tk.LEFT
        )
        ttk.Button(row1, text="Reload viewer", command=self.reload_changelog).pack(
            side=tk.LEFT, padx=8
        )
        ...
        ttk.Button(row2, text="Browse…", command=self.browse_config_file).pack(
            side=tk.LEFT
        )
    ...
    def build_left_panel(self, parent: ttk.Frame) -> None:
        ...
        for command in COMMANDS:
            ttk.Button(
                ...
                text=command,
                width=18,
                command=self.command_button_callback(command),
    ...
    def build_center_panel(self, parent: ttk.Frame) -> None:
        ...
        for command in COMMANDS:
            ...
            self.tab_frames[command] = frame
            self.build_command_tab(command, frame)

        # Changelog viewer tab
        cl_frame = ttk.Frame(self.notebook)
        self.notebook.add(cl_frame, text="changelog")
        self.tab_frames["changelog"] = cl_frame
        self.build_changelog_tab(cl_frame)

    def build_command_tab(self, command: str, frame: ttk.Frame) -> None:
        ...
        ttk.Button(
            run_row,
            text=f"Run {command}",
            command=self.run_command_callback(command),
        ).pack(side=tk.LEFT)
        ttk.Button(
            run_row,
            text="Clear output",
            command=self.clear_output_callback(command),
        ).pack(side=tk.LEFT, padx=6)

        output = scrolledtext.ScrolledText(frame, wrap=tk.WORD, height=20)
        output.pack(fill=tk.BOTH, expand=True, padx=6, pady=4)
        self.output_widgets[command] = output

    def build_changelog_tab(self, frame: ttk.Frame) -> None:
        toolbar = ttk.Frame(frame)
        toolbar.pack(fill=tk.X, padx=6, pady=4)
        ttk.Button(toolbar, text="Reload", command=self.reload_changelog).pack(
            side=tk.LEFT
        )
        ...
        self.changelog_view = scrolledtext.ScrolledText(frame, wrap=tk.WORD)
        self.changelog_view.pack(fill=tk.BOTH, expand=True, padx=6, pady=4)
    ...
    def build_right_panel(self, parent: ttk.Frame) -> None:
        ...
        self.help_text_widget.pack(fill=tk.BOTH, expand=True, padx=6, pady=6)

    # ------------------------------------------------------------------
    # Event handlers
    # ------------------------------------------------------------------
    def command_button_callback(self, command: str) -> Callable[[], None]:
        def callback() -> None:
            self.on_command_button(command)

        return callback

    def run_command_callback(self, command: str) -> Callable[[], None]:
        def callback() -> None:
            self.run_command(command)

        return callback

    def clear_output_callback(self, command: str) -> Callable[[], None]:
        def callback() -> None:
            self.clear_output(command)

        return callback

    def select_notebook_tab(self, tab: ttk.Frame) -> None:
        self.notebook.tk.call(str(self.notebook), "select", str(tab))

    def current_notebook_tab_text(self) -> str:
    ...
    def on_command_button(self, command: str) -> None:
        self.select_notebook_tab(self.tab_frames[command])

    def on_tab_changed(self, _event: Any) -> None:
        tab_text = self.current_notebook_tab_text()
        self.current_command = tab_text
        self.show_help(tab_text)

        if tab_text == "changelog":
            self.reload_changelog()
            return

        if tab_text in AUTO_RUN_COMMANDS and tab_text not in self.auto_run_done:
            self.auto_run_done.add(tab_text)
            self.run_command(tab_text)

    def show_help(self, command: str) -> None:
        if self.help_text_widget is None:
            return
        text = HELP_TEXT.get(command, "")
        self.help_text_widget.configure(state=tk.NORMAL)
        self.help_text_widget.delete("1.0", tk.END)
        self.help_text_widget.insert("1.0", text)
    ...
    def reload_changelog(self) -> None:
        if self.changelog_view is None:
            return
        path = self.input_file_var.get()
        logger.info("Reloading changelog viewer from %s", path)
        self.changelog_view.delete("1.0", tk.END)
        try:
            with Path(path).open(encoding="utf-8") as handle:
                self.changelog_view.insert("1.0", handle.read())
        except OSError as exc:
            logger.warning("Unable to reload changelog viewer from %s: %s", path, exc)
            self.changelog_view.insert("1.0", f"[unable to read {path}]\n{exc}\n")

    def clear_output(self, command: str) -> None:
        widget = self.output_widgets.get(command)
    ...
    # Command execution
    # ------------------------------------------------------------------
    def build_argv(self, command: str) -> list[str] | None:
        argv: list[str] = []
    ...
    def run_command(self, command: str) -> None:
        ...
        widget = self.output_widgets[command]
