...
def make_changelog_with_unreleased(
    change_types, released_version=None, file_path="CHANGELOG.md"
):
...
class TestAddInvariants:
    ...
    @given(
        ...
        messages=st.lists(changelog_message, min_size=2, max_size=10),
    )
    ...
    @given(
        ct1=valid_change_types,
        ct2=valid_change_types,
        msg1=changelog_message,
        msg2=changelog_message,
    )
    ...
    def test_add_does_not_remove_existing_released_versions(self, change_type, message):
        existing = OrderedDict(
            {"2.0.0": {"metadata": {"version": "2.0.0", "release_date": "2024-01-01"}}}
        )
        cl = Changelog(file_path="CHANGELOG.md", changelog=existing)
        cl.add(change_type, message)
...
class TestSuggestFutureVersionInvariants:
    ...
    @given(
        extra=st.frozensets(
            st.sampled_from(
                [t for t in TYPES_OF_CHANGE if t not in ("added", "removed")]
            )
        )
    )
    ...
    @given(extra=st.frozensets(st.sampled_from(["fixed", "changed", "deprecated"])))
    ...
    @given(
        change_types=st.frozensets(
            st.sampled_from(["fixed", "changed", "deprecated"]), min_size=1
        )
    )
    ...
    @given(
        change_types=change_type_subsets,
        major=st.integers(min_value=0, max_value=5),
        minor=st.integers(min_value=0, max_value=9),
        patch=st.integers(min_value=0, max_value=9),
    )
    def test_suggested_version_never_equal_to_current(
        self, change_types, major, minor, patch
    ):
...
class TestReleaseInvariants:
    ...
    def test_release_with_higher_version_removes_unreleased(
        self, change_types, override
    ):
...
class TestReleaseErrorCases:
    ...
    def test_release_already_released_version_raises(self, override):
        ...
        changelog[override] = {
            "metadata": {"version": override, "release_date": "2024-01-01"}
        }
    ...
    def test_release_older_than_current_raises(self, current, older):
        ...
        changelog[current] = {
            "metadata": {"version": current, "release_date": "2024-01-01"}
        }
        cl = Changelog(file_path="CHANGELOG.md", changelog=changelog)
        with pytest.raises(logging.Error):
...
class TestVersionQueryInvariants:
    ...
    def test_previous_version_single_raises(self, versions):
        changelog = {
            versions[0]: {
                "metadata": {"version": versions[0], "release_date": "2024-01-01"}
            }
        }
        cl = Changelog(file_path="CHANGELOG.md", changelog=changelog)
        with pytest.raises(logging.Warning):
