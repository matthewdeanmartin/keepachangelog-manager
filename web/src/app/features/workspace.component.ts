...
@Component({
  ...
  template: `
    <h1>Workspace</h1>
    <p class="hint">
      Choose where KATL reads and writes <code>tickets/</code> and
      <code>changelog.d/</code>. The active backend is
      <strong>{{ repo.backendId() }}</strong>.
    </p>
    ...
    <section class="card">
      ...
      <p class="warn">
        Paste a <strong>fine-grained PAT</strong> with Contents + Pull requests
        read/write. The token is kept <strong>in memory only</strong> and sent
        only to <code>api.github.com</code>. Refresh clears it.
      </p>
      <label>Token
        <input type="password" [(ngModel)]="token" placeholder="github_pat_… or ghp_…" autocomplete="off" />
      </label>
      <div class="row">
      ...
      @if (status()) { <p [class.err]="isError()" class="status">{{ status() }}</p> }
    </section>
  `,
  styles: [
    `
      .hint { color: #7b8794; font-size: 0.9rem; }
      .card { background: #f5f7fa; border-radius: 8px; padding: 1rem 1.25rem; margin-bottom: 1.25rem; }
      .card h2 { margin-top: 0; font-size: 1rem; }
      .warn { background: #fffbea; border: 1px solid #f0b429; border-radius: 6px; padding: 0.5rem 0.75rem; font-size: 0.8rem; }
      label { display: block; font-size: 0.8rem; color: #616e7c; margin-bottom: 0.6rem; }
      input { width: 100%; box-sizing: border-box; padding: 0.4rem; margin-top: 0.2rem; border: 1px solid #cbd2d9; border-radius: 4px; font: inherit; }
      .row { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 0.6rem; }
      button { background: #4da8da; color: #fff; border: 0; padding: 0.5rem 0.9rem; border-radius: 6px; cursor: pointer; }
      button:disabled { opacity: 0.4; cursor: not-allowed; }
      .status { font-size: 0.85rem; color: #2f855a; }
