...
1. **Implement Remote Backfill Sources**: Start with `github-releases` as it is high-value.
1. **Expand GUI Coverage**: Add the missing `to-html`, `remove`, `edit`, and `from-commits` commands to the GUI.
1. **Unified Interactive Layer**: Ensure all commands have a consistent Inquirer fallback when required arguments are missing in a TTY.
1. **Documentation Audit**: Clean up any remaining references to YAML or the `to-yaml` command.
